...
pub fn query_all_by_endpoint (conn: &PgConnection, endpoint: &str) -> Vec<DumpCollector> {
...
pub fn organize_much_data (conn: &DbUtils) {
    ...
    let data = query_all_by_endpoint(connection, "/dom/much_data");
    for dump in data {
...
pub fn organize_own_links (conn: &DbUtils) {
    let connection = &conn.get_connection();
    let data = query_all_by_endpoint(connection, "/dom/own_links");
    for dump in data {
...
pub fn organize_dom_xss(connection: &PgConnection) {
    use crate::schema::dom_xss::dsl::*;
    let data = query_all_by_endpoint(connection, "/dom/dom_xss");
    for dump in &data {
...
pub fn update_last_processed(connection: &PgConnection) {
    ...
    let last_index = Option::from((all.len() - 1).to_string());
    {
